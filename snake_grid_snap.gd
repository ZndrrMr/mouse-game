...
func _ready():
	print("Snake: Starting grid-snapped AStarGrid2D pathfinding system")
...
func _physics_process(delta):
	...
	# Follow the current path with grid snapping
	if not current_path.is_empty():
		_follow_path_grid_snapped(delta)
...
func _follow_path_grid_snapped(delta):
	...
	# Move to next waypoint if close enough (very small threshold for grid snapping)
	...
	# Move toward current target with smooth interpolation but snap to grid
	if current_target_index < current_path.size():
		...
		# If we're very close to target, snap to it exactly
