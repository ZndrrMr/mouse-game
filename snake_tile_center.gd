...
var last_target_tile: Vector2i = Vector2i(-999, -999)  # Track target tile, not player position

func _ready():
	print("Snake: Starting tile-center targeting pathfinding system")
...
func _physics_process(delta):
	...
	# Only recalculate path if player moved to a different tile
	if player_tile != last_target_tile:
...
func _calculate_path_to_player_tile(target_tile: Vector2i):
	...
	print("Snake: Pathfinding from ", start_map, " to ", end_map, " (player tile center)")
	...
	print("Snake: Found path with ", current_path.size(), " points to player tile center")
