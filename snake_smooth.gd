...
var path_recalc_interval = 1.0  # Recalculate path every 1 second (was 0.5)
var movement_target: Vector2
var is_moving_to_target = false
var last_player_grid_pos: Vector2i = Vector2i(-999, -999)

func _ready():
	print("Snake: Starting smooth grid-snapped pathfinding system")
...
func _physics_process(delta):
	...
	# Only recalculate if player moved to a different grid cell or enough time passed
	var player_local = tilemap_layer.to_local(player.global_position)
	var player_grid_pos = tilemap_layer.local_to_map(player_local)

	var should_recalc = (
		current_path.is_empty() or
		path_recalc_timer >= path_recalc_interval or
		player_grid_pos != last_player_grid_pos
	)

	if should_recalc:
		path_recalc_timer = 0.0
		last_player_grid_pos = player_grid_pos
