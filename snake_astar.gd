...
const TILE_SIZE = 128  # Size of each tile
const GRID_TOLERANCE = 16  # How close to grid center before moving to next point
	...
	for x in range(grid_width):
		for y in range(grid_height):
			...
			var neighbors = [
				...
				Vector2(x, y + 1),  # Down
				Vector2(x, y - 1),  # Up
				Vector2(x + 1, y + 1),  # Diagonal down-right
				Vector2(x - 1, y - 1),  # Diagonal up-left
				Vector2(x + 1, y - 1),  # Diagonal up-right
				Vector2(x - 1, y + 1)   # Diagonal down-left
			]
...
func _should_recalculate_path() -> bool:
	...
	# Recalculate if player moved to a different grid cell
	var player_grid = _world_to_grid(player.global_position)
	var target_grid = _world_to_grid(current_path[-1])

	return player_grid != target_grid
...
func _calculate_path_to_player():
	...
	if current_path.size() > 1:
		...
		print("Snake: No path found to player")
